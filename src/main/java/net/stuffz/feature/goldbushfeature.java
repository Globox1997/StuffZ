  ...
  public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
    ...
    for (int i = -1; i < 2; i++) {
      for (int u = -1; u < 2; u++) {
        ...
        isAirOrGrass = context.getWorld().getBlockState(topPos.north(i).east(u)).isAir()
            || context.getWorld().getBlockState(topPos.north(i).east(u)).getBlock().equals(Blocks.GRASS);
        if (isGrassBlock && isAirOrGrass) {
          booleanCount++;
        }
        if (booleanCount == 9) {
          int randomInt = context.getWorld().getRandom().nextInt(3) + 1;
          for (int k = 0; k < randomInt; k++) {
            ...
            context.getWorld().setBlockState(topPos.east(randomIntEast).south(randomIntSouth),
