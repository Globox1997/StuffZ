...
public class nethergeyserblock extends Block {
    ...
    public void onSteppedOn(World world, BlockPos pos, Entity entity) {
        ...
        while (count2 < 60) {
            ...
            count3 = count2 / 10D;
            world.addParticle(ParticleTypes.CLOUD, pos.getX() + 0.5D, pos.getY() + count3 + 1D, pos.getZ() + 0.5D, 0.0D,
                    0.0D, 0.0D);
            if (count2 < 10) {
                world.addParticle(ParticleTypes.FLAME, pos.getX() + 0.3D + z2, pos.getY() + 1D, pos.getZ() + 0.3D + z4,
                        0.0D, 0.0D, 0.0D);
            }
            if (count2 < 25) {
