...
public class BrewingBarrelRenderer implements BlockEntityRenderer<BrewingBarrelEntity> {
    ...
    @Override
    public void render(BrewingBarrelEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if (!state.isAir()) {
            ...
            if (blockDirection.equals(Direction.EAST)) {
                ...
                if (!blockEntity.getStack(1).isEmpty()) {
                    ...
                    MinecraftClient.getInstance().getItemRenderer().renderItem(blockEntity.getStack(1), ModelTransformation.Mode.GROUND, lightAbove, overlay, matrices, vertexConsumers,
                            (int) blockEntity.getPos().asLong());
                    matrices.pop();
                }
                if (!blockEntity.getStack(4).isEmpty()) {
                    ...
                    matrices.translate(0.5D, 0.26D, 0.96D);
                    matrices.scale(0.8F, 0.8F, 0.8F);
                    MinecraftClient.getInstance().getItemRenderer().renderItem(blockEntity.getStack(4), ModelTransformation.Mode.GROUND, lightAbove, overlay, matrices, vertexConsumers,
                            (int) blockEntity.getPos().asLong());
                    matrices.pop();
                }
            }
