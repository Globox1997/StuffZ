...
public class FeatureInit {
    ...
    public void init() {
        ...
        Biomes.NETHER_WASTES.addFeature(GenerationStep.Feature.VEGETAL_DECORATION,
                NETHERGEYSER_FEATURE.configure(FeatureConfig.DEFAULT).createDecoratedFeature(
                        Decorator.COUNT_CHANCE_HEIGHTMAP.configure(new CountChanceDecoratorConfig(3, 0.1F))));
        Biomes.GRAVELLY_MOUNTAINS.addFeature(GenerationStep.Feature.VEGETAL_DECORATION,
                STONEGEYSER_FEATURE.configure(FeatureConfig.DEFAULT).createDecoratedFeature(
                        Decorator.COUNT_CHANCE_HEIGHTMAP.configure(new CountChanceDecoratorConfig(2, 0.1F))));
        Biomes.MOUNTAINS.addFeature(GenerationStep.Feature.VEGETAL_DECORATION,
                STONEGEYSER_FEATURE.configure(FeatureConfig.DEFAULT).createDecoratedFeature(
                        Decorator.COUNT_CHANCE_HEIGHTMAP.configure(new CountChanceDecoratorConfig(2, 0.05F))));
    }
    ...
    private void handleBiome(Biome biome) {
        ...
        if (category == Biome.Category.EXTREME_HILLS || category == Biome.Category.SWAMP
            ...
            biome.addFeature(GenerationStep.Feature.UNDERGROUND_ORES,
                    Feature.ORE.configure(new OreFeatureConfig(OreFeatureConfig.Target.NATURAL_STONE,
                            BlockInit.SULFURBLOCK.getDefaultState(), 5)).createDecoratedFeature(
        ...
        if (category == Biome.Category.OCEAN) {
            biome.addFeature(GenerationStep.Feature.UNDERGROUND_ORES,
                    Feature.ORE.configure(new OreFeatureConfig(OreFeatureConfig.Target.NATURAL_STONE,
                            BlockInit.FOSSILBLOCK.getDefaultState(), 1)).createDecoratedFeature(
        ...
        if (category == Biome.Category.OCEAN || category == Biome.Category.ICY) {
            biome.addFeature(GenerationStep.Feature.UNDERGROUND_ORES,
                    Feature.ORE.configure(new OreFeatureConfig(OreFeatureConfig.Target.NATURAL_STONE,
                            BlockInit.YELLOWRUBYBLOCK.getDefaultState(), 1)).createDecoratedFeature(
